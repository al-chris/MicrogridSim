...
%CSA_Adaptive Cuckoo Search with adaptive Lévy scaling
    dim = numel(lb); np = opts.n_nests; maxIter = opts.max_iter;
    alpha = opts.alpha0; stagnWindow = 10;
    nests = rand(np,dim).*(ub'-lb') + lb'; fitness = inf(np,1);
    for i=1:np, fitness(i)=fitFunc(nests(i,:)'); end
    ...
    for iter=1:maxIter
        for i=1:np
            ...
            cost = fitFunc(new'); j=randi(np);
            if cost<fitness(j), nests(j,:)=new; fitness(j)=cost; end
        end
        [~,worst]=maxk(fitness,floor(0.25*np));
        for w=worst', nests(w,:)=rand(1,dim).*(ub'-lb')+lb'; fitness(w)=fitFunc(nests(w,:)'); end
        [currBest, idx]=min(fitness); if currBest<bestCost, bestCost=currBest; bestSol=nests(idx,:)'; end
